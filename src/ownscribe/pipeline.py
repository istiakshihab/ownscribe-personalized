...
def _create_transcriber(config: Config, progress=None):
    """Create the MLX Whisper transcriber."""
...
def run_warmup(config: Config) -> None:
    ...
    with PipelineProgress(
        ...
        try:
            ...
            click.echo(
                "Error: mlx-whisper is not installed. Install with:\n  uv pip install mlx-whisper",
                err=True,
            )
...
def _do_transcribe_and_summarize(
    ...
    if vault_prepared is not None:
        ...
        parsed = vault_prepared.parsed
        should_write = True
        if config.obsidian.confirm_before_filing and sys.stdin.isatty():
            click.echo("\n--- Proposed vault note ---")
            click.echo(f"Title:    {parsed.get('title', '')}")
            click.echo(f"Category: {parsed.get('category', '')}")
            click.echo(f"Tags:     {', '.join(parsed.get('tags', []))}")
            click.echo(f"\n{parsed.get('body_markdown', '')}\n")
            should_write = click.confirm("File this into the Obsidian vault?", default=True)

        if should_write:
            try:
                vault_note = write_note(
                    parsed, vault_prepared.folder_name, vault_prepared.date, config.obsidian.resolved_vault_dir
                )
            except VaultFilingError as exc:
                vault_error = str(exc)
        else:
    ...
    if vault_note is not None:
        click.echo(
            f"Filed to vault: {vault_note.path}"
            + (" [NEEDS REVIEW - unrecognized category]" if vault_note.needs_review else "")
        )
    elif vault_error is not None:
        click.echo(f"Warning: vault filing failed: {vault_error}", err=True)
...
# Video containers whose audio track ffmpeg/mlx-whisper can decode (e.g. meeting recordings).
