...
confirm_before_filing = true  # show the proposed note and ask before writing it (skipped when not a TTY)
"""
...
class ObsidianConfig:
    enabled: bool = True
    vault_dir: str = "~/Documents/obsidian/Personal"
    claude_bin: str = ""  # empty = auto-detect via PATH
    confirm_before_filing: bool = True
...
def _merge_toml(config: Config, data: dict) -> Config:
    ...
    if "obsidian" in data:
        for k, v in data["obsidian"].items():
            if hasattr(config.obsidian, k):
                setattr(config.obsidian, k, v)
